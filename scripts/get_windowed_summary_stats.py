...
fdfm = pd.DataFrame(l_data_masked, columns=['position', 'genotype'])
fdfm['pos'] = [int(x)+1 for x in (np.round(fdfm['position']*chr_len, 0))]
#Determine if site is non-synonymous or synonymous
fdfm['csq'] = np.where(fdfm.pos%3==0, 'S', 'NS')

fdf1 = fdf[fdf.pos<=10000].copy()
#Create nested lists for synonymous and non-synonymous sites
S = fdf1[fdf1.csq=='S'][['pos', 'genotype']].values
NS = fdf1[fdf1.csq=='NS'][['pos', 'genotype']].values

fdf1m = fdfm[fdfm.pos<=10000].copy()
#Create nested lists for synonymous and non-synonymous sites
S2 = fdf1m[fdf1m.csq=='S'][['pos', 'genotype']].values
NS2 = fdf1m[fdf1m.csq=='NS'][['pos', 'genotype']].values
...
dfN = pd.DataFrame.from_dict(d_NS, orient='index').T
dfN = dfN[['thetapi']]
dfN['num_sites'] = len(NS2)
dfN.columns = [1,2]
dfS = pd.DataFrame.from_dict(d_S, orient='index').T
dfS = dfS[['thetapi']]
dfS['num_sites'] = len(S2)
dfS.columns = [1,2]


fdf2 = fdf[(fdf.pos>10000) & (fdf.pos<=20000)].copy()
#Create nested lists for synonymous and non-synonymous sites
S = fdf2[fdf2.csq=='S'][['pos', 'genotype']].values
NS = fdf2[fdf2.csq=='NS'][['pos', 'genotype']].values

fdf2m = fdfm[(fdfm.pos>10000) & (fdfm.pos<=20000)].copy()
#Create nested lists for synonymous and non-synonymous sites
S2 = fdf2m[fdf2m.csq=='S'][['pos', 'genotype']].values
NS2 = fdf2m[fdf2m.csq=='NS'][['pos', 'genotype']].values
...
d_S = get_polySIM_stats(sd_S)
#Convert dict to dataframe and append to file
dfN2 = pd.DataFrame.from_dict(d_NS, orient='index').T
dfN2 = dfN2[['thetapi']]
dfN2['num_sites'] = len(NS2)
dfN2.columns = [3,4]
dfS2 = pd.DataFrame.from_dict(d_S, orient='index').T
dfS2 = dfS2[['thetapi']]
dfS2['num_sites'] = len(S2)
dfS2.columns = [3,4]
dfN = dfN.join(dfN2)
dfS = dfS.join(dfS2)


fdf3 = fdf[(fdf.pos>20000)].copy()
#Create nested lists for synonymous and non-synonymous sites
S = fdf3[fdf3.csq=='S'][['pos', 'genotype']].values
NS = fdf3[fdf3.csq=='NS'][['pos', 'genotype']].values

fdf3m = fdfm[(fdfm.pos>20000)].copy()
#Create nested lists for synonymous and non-synonymous sites
S2 = fdf3m[fdf3m.csq=='S'][['pos', 'genotype']].values
NS2 = fdf3m[fdf3m.csq=='NS'][['pos', 'genotype']].values
...
d_S = get_polySIM_stats(sd_S)
#Convert dict to dataframe and append to file
dfN3 = pd.DataFrame.from_dict(d_NS, orient='index').T
dfN3 = dfN3[['thetapi']]
dfN3['num_sites'] = len(NS2)
dfN3.columns = [5,6]
dfS3 = pd.DataFrame.from_dict(d_S, orient='index').T
dfS3= dfS3[['thetapi']]
dfS3['num_sites'] = len(S2)
dfS3.columns = [5,6]
dfN = dfN.join(dfN3)
dfS = dfS.join(dfS3)


dfN.to_csv(out_path + '_NS.txt', sep='\t', index=False, header=False, mode='a')
dfS.to_csv(out_path + '_S.txt', sep='\t', index=False, header=False, mode='a')
